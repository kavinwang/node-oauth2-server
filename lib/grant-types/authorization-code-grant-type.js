...
AuthorizationCodeGrantType.prototype.getAuthorizationCode = function(request, client) {
  if (!request.body.code && !request.body.authCode) {
    throw new InvalidRequestError("Missing parameter: `code` or `authCode`");
  }
  //because we support phonenum and authcode
  if (!is.vschar(request.body.code || request.body.authCode)) {
    throw new InvalidRequestError("Invalid parameter: `code` or `authCode`");
  }

  return promisify(this.model.getAuthorizationCode, 2).call(this.model, request.body.code || request.body.authCode, request.body.phoneNum)
    .then(function(code) {
      if (!code) {
